...
Em 1934, Leray construiu soluções fracas globais de energia finita $\bu(\cdot,t) \in \cL^\infty\big([0,\infty), \cL^2_\sigma(\bR^3)\big) \cap \cC_W\left([0,\infty), \cL^2(\bR^3)\right) \cap \cL^2\big([0,\infty), \dot\cH(\bR^3)\big)$\footnote{Os espaços (...)} para as equações de Navier-Stokes em $\bR^3$
...
\textcolor{red}{(seção 2 :: paulo)}

(...)

\[
    \partial_t \bu_\delta + \bar \bu_\delta(\cdot,t) \geq \nabla \bu_\delta + \nabla \bu_\delta + \nabla p_\delta= \nu \Delta \bu_\delta
\]
...
\begin{pbox}
    ...
    \]
    para todo $t > s$, onde $c = (8\pi)^{-\frac{3}{4}}$.
\end{pbox}
\begin{prf}
...
\begin{prf}
    ...
    \[
        \bv(\cdot,t) = e^{\nu\Delta(t-t_0)} (\bu(\cdot,t) - \bu_\delta(\cdot,t)) + e^{\nu\Delta(t-t_0)}\bu_\delta(\cdot,t_0)
    \]
...
\begin{tbox}[Solução do problema clássico de Leray]
    Seja $\bu(\cdot,t)$ uma solução de Leray para (\ref{eq:navierstokes}), então
    \[
