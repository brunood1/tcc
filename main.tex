...
\subsection{Operadores lineares limitados}
...
\[
    \nabla u(x,t) = 0
\]
\chapter{Soluções fracas das equações de Navier-Stokes}

\section{Introdução}

\section{Preliminares}

Alguns resultados que vamos utilizar já foram apresentados em outras partes do texto, então aqui vamos apresentar novos preliminares

\begin{pbox}[Desigualdade de Young para Convoluções]
    Sejam $f \in \cL^p(\bR^n)$ e $g \in \cL^q(\bR^n)$, então $f * g \in \cL^r$ com $1 + \frac{1}{r} = \frac{1}{p} + \frac{1}{q}$ e
    \[
        \Vert f * g \Vert_{\cL^r(\bR^n)} \leqslant \Vert f \Vert_{\cL^p(\bR^n)} \Vert g \Vert_{\cL^q(\bR^n)}.
    \]
\end{pbox}

\section{Movimento infinitamente lento}

\[
    \tilde u_i(x,t) = \frac{1}{(2\sqrt{\pi})^3} \int_{\bR^3} \frac{e^{-\frac{r}{4\nu t}}}{(\nu t)^{\frac{3}{2}}} u_i(y,0) \,dy
\]
