...
\begin{tbox}
    Sejam $\Omega$ um aberto limitado com fronteira $\partial\Omega$ de classe $\cC^1$ e $u \in \cW^{k,p}(\Omega)$. 
    Se $kp > n$, então $u \in \cC^{K,\gamma}(\overline\Omega)$, onde
    \[
        K = k - \left\lfloor \frac{n}{p} \right\rfloor - 1
    \]
    e
    \[
        \gamma = \left\lfloor \frac{n}{p} \right\rfloor + 1 - \frac{n}{p}
    \]
    se ${}^{n\!}/_{\!p}$ não é um inteiro e $\gamma \in (0,1)$ se ${}^{n\!}/_{\!p}$ é um inteiro.
    Além disso a desigualdade
    \[
        \Vert u \Vert_{\cC^{K,\gamma}(\overline\Omega)} \leqslant c \Vert u \Vert_{\cW^{k,p}(\Omega)}
    \]
