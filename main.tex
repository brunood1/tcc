...
\begin{prf}
    ...
    \]
    Ou seja, podemos definir para cada $\delta > 0$ \dots
...
\begin{prf}
    ...
    \[
        \Vert u \Vert_{\cL^{p^*}(\bR^n)}^{p^*} \int_{\bR^n} |u|^{\frac{n}{n-1}} \, dx \leqslant \prod_{i=1}^n \left( \int_{\m\infty}^{\infty} \cdots \int_{\m\infty}^\infty |Du| dx_1\dots dy_i \dots dx_n \right)^{\frac{1}{n-1}} = \left(\int_{\bR^n} |Du|\,dx\right)^{\frac{n}{n-1}} = \Vert Du \Vert_{\cL^1(\bR^n)}^{p^*}
    \]
    Ou seja
...
\obs (suporte compacto é necessário)
