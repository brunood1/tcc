...
\begin{prf}
    ...
    Observe que
    \[
        \sint_{B(x,1)} |u(y)| \,dy = \frac{1}{\sigma(n)} \int_{B(x,1)} |u(y)| \,dy \leqslant c \left( \int_{B(x,1)} |u(y)|^p\,dy \right)^{\frac{1}{p}} \left( \int_{B(x,1)} 1^{p'} \,dy \right)^{\frac{1}{p'}} \leqslant c \Vert u \Vert_{\cL^p(\bR^n)}
    \]
    Além disso
    \[
    ...
    Portanto por () e ()
    \[
        \Vert u \Vert_{\cC^{0,\gamma}(\bR^n)} \leqslant c \Vert Du \Vert_{\cL^p(\bR^n)}.
    \]
...
\chapter{Derivando as equações de Navier-Stokes}

Nesse capítulo visamos derivar as equações de Navier-Stokes 
\[
    \begin{array}{c}
        \bu_t + \bu \cdot \nabla \bu + \nabla p = \nu \Delta \bu + \bX\\
        \nabla \cdot \bu = 0
    \end{array}
\]
partindo do principio de conservação de energia \textcolor{red}{(?)} e ressaltar alguns fatos importantes sobre essas equações e suas soluções.
