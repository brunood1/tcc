...
\begin{tbox}[Propiedades da transformada de Fourier] \label{thm:propriedades-transformada}
    Seja $u, v \in \cL^2(\bR^n)$. Então, são válidas as seguintes igualdades
    \begin{enumerate}[leftmargin=*, label=\textbf{(\alph*)}]
...
\[
    e^{\nu \Delta \tau} \mathbf{x} = \frac{\mathbf{x} * E(\cdot,\tau)}{(4\pi \nu \tau)^{\frac{n}{2}}}
\]
...
Em 1934 no artigo \textit{``Sur le mouvement d'un liquide visqueux emplassement l'espace''} (ver \cite{leray-fluid}) Leray construiu soluções fracas de energia finita\footnote{i.e., \textcolor{red}{qual é o significado de energia finita?}}
...
\begin{equation} \label{eq:navier-stokes-regularizado}
    \begin{aligned}
       ...
       \end{aligned}
\end{equation}
onde $\bar\bu_\delta = \eta_\delta * \bu_\delta$. Em seu artigo, Leray mostrou que existe uma sequência apropriada $\delta' \to 0$ tal que conseguimos a convergência fraca em $\cL^2(\bR^3)$
\begin{equation} \label{eq:b1}
    \bu_{\delta'} \rightharpoonup \bu
\end{equation}
para todo $t \geqslant 0$, onde $\bu(\cdot,t)$ apresentada em (\ref{eq:solucao-leray}) contínua no instante $t = 0$.
...
\begin{prf}
    ...
    \]
    onde utlizamos o resultado sobre a transformada de Fourier do semigrupo do calor visto nos preliminares. Além disso, $\Vert \hat \bv(\omega,s) \Vert^2 \leqslant \Vert \hat\bv(\cdot,s) \Vert_\infty = \sup\{ \Vert \hat\bv(\omega,s) \Vert \,; \omega \in \bR^3 \}$. Logo
    \[
    ...
    \]
    Deste modo, pela desigualde de Cauchy-Schwarz, podemos escrever, $\Vert \hat\BQ(\omega,s) \Vert^2 \leqslant \Vert \cF [\bu(\cdot,s) \cdot \nabla \bu(\cdot,s)] \Vert \Vert \hat\BQ \Vert$. Ou seja,
    \[
    ...
    Por fim, novamente utliizando a Desigualdade de Hölder (Cauchy-Schwarz) e a definição das normas, chegamos a
    \[
...
\begin{prf}
    ...
    Como $\bu(\cdot,t)$ é suave em $[t_0,\infty)$ (ver Seção ??) obtemos, de forma análoga ao que foi feito na demonstração da Proposição \ref{pr:dav}
    \[
    ...
    \[
        ...
        \end{aligned}
    \]
    Como $e^{-2\nu (t-t_0) \Vert \omega \Vert^2} \Vert \hat\bu(\omega,t_0) \Vert^2 \leqslant \Vert \hat\bu(\omega,t_0) \Vert^2 \in \cL^1(\bR^3)$, então, pelo Teorema da Convergência Dominada
    \[
        \Vert e^{\nu \Delta (t-t_0)} \bu(\cdot,t_0) \Vert_{\cL^2(\bR^3)} \to 0
    \]
...
\[
    \Vert \bu(\cdot,t) \Vert_{H^1(\bR^3)}^2 = \Vert \bu(\cdot,t_0) \Vert_{\cL^2(\bR^3)}^2 + t^{-\frac{1}{2}} \big[ t^{\frac{1}{4}} \Vert D\bu(\cdot,t) \Vert_{\cL^2(\bR^3)}\big]^2 \to 0 
\]
quando $t \to \infty$.
...
\begin{prf}
    ...
    \]
    para todo $t > 0$ (em particular para todo $t \geqslant T_{**}$). Logo, $U_0$ é limitada e $U_0 \in \cL^{\infty}([T_{**}, \infty))$.
    ...
    \[
        |I_2(\alpha,t)|\leqslant c(k,\nu) M_1 \Vert \bu_0 \Vert_{\cL^2(\bR^3)} (t-t_0)^{-\left( \frac{k}{2} + \frac{3}{4} \right)} \int_{t_0}^{t'} s^{-\frac{1}{2}} \,ds.
    \]
    Sendo assim, por (*), chegamos a
    \[
...
\begin{prf}
    ...
    Como $f$ é um funcional linear limitado em $H$, pelo Teorema da Representação de Riesz (ver Teorema \ref{thm:representacao-riesz}), existe $z \in H$ tal que $f(u) = \left\langle u, z \right\rangle$ para todo $u \in H$.
