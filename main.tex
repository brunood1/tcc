...
\begin{prf}
    ...
    \begin{figure}
        \centering
        \textcolor{red}{FIGURA DO HOMEOMORFISMO $\Phi$}
        \caption{~}
    \end{figure}
...
\begin{prf}
    ...
    \]
    (finalizar, preciso refazer o final e o pedacinho de $D(u \circ \Psi)$)
\end{prf}
...
\begin{prf}
    ...
    \]
    como era desejado.
    Por fim, escolhendo $\delta = 1,\frac{1}{2},\dots$ em (), conseguimos extrair uma subsequência $(u_{k_j})_{j=1}^\infty \subseteq (u_k)_{k=1}^\infty$ que satisfaz
    \[
...
\section{Estimativas de decaimento das soluções das equações de Navier-Stokes}
