...
\begin{tbox}[Coordenadas Polares em $\bR^n$]
    Seja $u : B(x_0,r) \subseteq \bR^n \to \bR$ uma função contínua, então
    \[
        \int_{B(x_0,s)} f \,dx = \int_0^s\int_{\partial B(x_0,r)} f \,dS dr
    \]
\end{tbox}
\begin{prf}
...
\begin{tbox} \label{thm:teorema-do-brezis}
    Seja $(u_n)$ uma sequência em $\cL^p(\Omega)$ tal que $\Vert u_n - u \Vert_{\cL^p(\Omega)} \to 0$ para alguma função $u \in \cL^p(\Omega)$.
    Então existe uma subsequência $(u_{n_k})$ e uma função $v \in \cL^p(\Omega)$ tal que
...
\begin{prf}
    ...
    \[
        D^\alpha u = u_\alpha.
    \]
Com efeito, pelo Teorema \ref{thm:teorema-do-brezis}, o \nameref{thm:teorema-da-convergencia-dominada} e utlizando a definição de derivada fraca, obtemos \textcolor{red}{(precisa falar ``passando a uma subseq se necessário?'')}
    \[
        \begin{aligned}
...
\begin{prf}
    ...
    \[
        \Vert D^\alpha \bar u' \Vert_{\cL^p(B)}^p = \int_B |D^\alpha \bar u'(y)|^p \,dy = \int_B |D^\alpha \bar u (\Psi (y))|^p \,dy = \int_W |D^\alpha \bar u (x)|^p \,dx = \Vert D^\alpha \bar u \Vert_{\cL^p(B')}^p.
    \]
...
\begin{prf}
    ...
    Sendo assim, seja $B = B(\tilde x, r)$ (e defina $B^+$ e $B^-$ como na demonstração anterior) e $\widehat B = B(\tilde x, \sfrac{r}{2})$.
    ...
    \]
    onde $\nu$ é o vetor unitário que aponta para baixo em (?), isto é $\nu = (0,\dots,0,-1)$. Sendo assim, $\nu_n = -1$ e 
    \[
    ...
    \]
    onde utilizamos o fato de $\xi$ ter suporte compacto para a ultima desigualdade. Por fim utilizando a Desigualdade de Young
    ...
    \]
    onde $c$ é uma constante que depende de $\gamma$. Dessa forma por (\ref{eq:umaisdu})
    \[
        \begin{aligned}
    ...
    Seja $\{\phi_i\}_{i=1}^N$ uma partição da unidade subordinada a cobertura $\{\Psi(\Gamma_i)\}_{i=1}^N$. Sendo assim, $u = \sum_{i=1}^N \phi_i u$.
    ...
    \[
        \begin{aligned}
            ...
            &= c \sum_{i=1}^N \int_{\Psi(\Gamma_i)} |u|^p \leqslant c\Vert u \Vert_{\cW^{1,p}(\Omega)}^p
        \end{aligned}
    \]
    isto mostra que $\widetilde T$ é um operador limitado.
    ...
    \]
    \textcolor{red}{(finalizar, preciso refazer o final e o pedacinho de $D(u \circ \Psi)$)}
\end{prf}
...
\begin{prf}
    ...
    Seja $x' \in \bR^{n-1}$ e $x_n \geqslant 0$.
    ...
    \]
    Utlizando a Desigualdade de Hölder na ultima integral, temos que o lado direito da equação acima é menor igual a
    \[
    ...
    \]
    Utlizando o Teorema da Convergência Dominada quando $k \to \infty$.
    ...
    \[
        ...
        \end{aligned}
    \]
    e defina $\xi_k = \xi(kx_n)$ e $w_k = u(x) (1 - \xi_k(x))$ para todo $x \in \bR^n_+$. Dessa forma
    \[
        \begin{aligned}
            &\dfrac{\partial w_k}{\partial x_n} = (1 - \xi_k)\dfrac{\partial u}{\partial x_n} - ku\xi'\\
            &\dfrac{\partial w_k}{\partial x'} = (1-\xi_k)\dfrac{\partial u}{\partial x'}.
        \end{aligned}
    \]
    Consequentemente
    \[
        \begin{aligned}
            \int_{\bR^n_+} \Vert Dw_k - Du \Vert^p \,dx &\leqslant c \int_{\bR^n_+} \left( \left\Vert \dfrac{\partial w_k}{\partial x'} - \dfrac{\partial u}{\partial x'} \right\Vert + \left| \dfrac{\partial w_k}{\partial x_n} - \dfrac{\partial u}{\partial x_n} \right| \right)^p dx\\
            &\leqslant c  \int_{\bR^n_+} \left( \xi_k\left\Vert \dfrac{\partial u}{\partial x'} \right\Vert + \xi_k \left| \dfrac{\partial u}{\partial x_n}\right| + m |u| |\xi'|  \right)^p dx\\ 
        \end{aligned}
    \]
    (explicar)
    \[
        \int_{\bR^n_+} \Vert Dw_k - Du \Vert^p \,dx \leqslant c \int_{\bR^n_+} \xi_k^p \Vert Du \Vert^p dx + cm^p \int_{\bR^n_+} |\xi_k'|^p |u|^p dx
    \]
    ...
    \[
        \int_{\bR^n_+} \Vert Dw_k - Du \Vert^p \,dx \leqslant c \int_{\bR^{n-1}} |\xi_k|^p \Vert Du \Vert^p \,dx + cm^p \int_{0}^{\frac{2}{k}} \int_{\bR^{n-1}} |u|^p \,dx'dt.
    \]
    Note que
    \[
        \begin{aligned}
    ...
    Portanto
    \[
        \int_{\bR^n_+} \Vert Dw_k - Du \Vert^p \,dx \to 0
    \]
    isto é $\Vert Dw_k - Du \Vert_{\cL^p(\bR^n_+)} \to 0$ quando $k \to 0$.

    Por fim
    \[
        \Vert w_k - u \Vert_{\cL^p(\bR^n_+)}^p = \int_{\bR^n_+} |w_m - u|^p \,dx = \int_{\bR^n_+} \xi_k|u|^p \,dx.
    \]
    De forma análoga ao que foi feito anteriormente
    \[
        \Vert w_k - u \Vert_{\cL^p(\bR^n_+)}^p = \int_{0}^{\frac{2}{k}} \xi(mx_n) \int_{\bR^{n-1}} |u(x)|^p \,dx'dx_n \leqslant c \int_{0}^{\frac{2}{k}} \int_{\bR^{n-1}} |u(x)|^p \, dx'dx_n \to 0.
    \]
    Dessa forma
    \[
        \Vert w_n - u \Vert_{\cW^{1,p}(\bR^n_+)}^p \leqslant \Vert w_n - u \Vert_{\cL^{p}(\bR^n_+)}^p + \Vert Dw_n - Du \Vert_{\cL^{p}(\bR^n_+)}^p \to 0
    \]

    Defina $u_k = \eta_{\frac{1}{k}} * w_k$. Assim, para $k$ grande $u_k \in \cC^{\infty}_c(\bR^n_+)$ e 
    \[
        \Vert u_k - u \Vert_{\cW^{1,p}(\bR^n_+)} = \Vert u_k - w_k \Vert_{\cW^{1,p}(\bR^n_+)} + \Vert w_k - u \Vert_{\cW^{1,p}(\bR^n_+)} \to 0
    \]
...
\begin{prf}
    ...
    Afirmamos que existe uma constante $c$ dependendo apenas de $n$ tal que
    \begin{equation} \label{eq:desigualdade-uyux}
        \sint_{B(x,r)} |u(y) - u(x)| \,dy \leqslant c \int_{B(x,r)} \frac{\Vert Du(y) \Vert}{|y-x|^{n-1}} \,dy.
    \end{equation}
    Com efeito, fixe $w \in \partial B(\bz,1)$, assim, se $0 < s < r$ segue que
    \[
    ...
    \]
    Seja $y = x + tw$, de forma que $t = \Vert x - y \Vert$. Assim, obtemos
    \[
    ...
    \]
    Por fim dividindo ambos os lados por $\sigma(n) r^n$ (volume da $n$-esfera de raio $r$)
    \[
...
\end{tbox}
\begin{prf}
    Suponha que $\sfrac{n}{p}$ não é um inteiro. Então como visto na demonstração anterior temos que $u \in \cW^{k-l,r}(\Omega)$ quando
    \[
        \frac{1}{r} = \frac{1}{p} - \frac{l}{n}
    \]
    desde que $lp < n$. Além disso, $l$ é um inteiro tal que
    \[
        l \leqslant \frac{n}{p} < l + 1
    \]
    isto é $l = \left\lfloor \frac{n}{p} \right\rfloor$.
    Consequentemente temos que
    \[
        r = \frac{pn}{n-pl} > n
    \]
    assim, pela Desigualdade de Morrey temos que $D^\alpha u \in \cC^{0,1-\frac{n}{r}}(\overline\Omega)$ para todo $|\alpha| \leqslant k - l - 1$. Observe também que $1 - \frac{n}{r} = l +1 - \frac{n}{p} = \gamma$.
    Portanto $u \in \cC^{k-l-1,\gamma}(\overline\Omega)$ e
    \[
        \text{desigualdade}
    \]

    Por fim, suponha que $\sfrac{n}{p}$ é um inteiro. Denotando $l = \sfrac{n}{p} - 1$, temos de forma análoga ao caso anterior que $u \in \cW^{k-l,r}(\Omega)$ com $r = \frac{pn}{n-pl} = n$.
    Assim, utilizando a Desigualdade de Gagliardo-Nirenberg-Sobolev temos que
    $D^\alpha u \in \cL^q(\Omega)$ para todo $n \leqslant q < \infty$ e $|\alpha| \leqslant k - l - 1$.
    Novamente utilizando a Desigualdadede Morrey, $D^\alpha u \in \cC^{0,1-\frac{n}{q}}(\overline\Omega)$ para todo $n < q < \infty$ e $|\alpha| \leqslant k - l - 1$.
    Consequentemente $u \in \cC^{k-l-1,\gamma}(\overline\Omega)$ para todo $0 < \gamma < 1$ e
    \[
        \text{desigualdade}
    \]
    Finalizando a demonstração
...
\begin{prf}
    ...
    Como $\Omega$ é limitado temos pelo Teorema \ref{thm:desigualdade-teorema-2} temos que $\cW^{1,p}(\Omega) \subseteq \cL^q(\Omega)$ e $\Vert u \Vert_{\cL^q(\Omega)} \leqslant c \Vert u \Vert_{\cL^{p^*}(\Omega)} \leqslant c \Vert u \Vert_{\cW^{1,p}(\Omega)}$.
    Resta mostrar que se $(u_k)_{k = 1}^\infty$ é uma sequência limitada em $\cW^{1,p}(\Omega)$, então existe uma subsequência $(u_{k_j})_{j=1}^\infty$.

    Pelo Teorema de Extensão, podemos supor sem perda de generalidade que $\Omega = \bR^n$ e que para todo $k \in \bN$, $u_k$ tem suporte compacto em algum aberto limitado $\Omega' \subseteq \bR^n$. 
    Também podemos supor que
    \begin{equation} \label{eq:supfinito}
        \sup_k \Vert u_k \Vert_{\cW^{1,p}(\Omega')} < \infty
    \end{equation}

    Primeiramente vamos estudar as funções suavizadas $u_k^\varepsilon = \eta_\varepsilon * u_k$ onde $\eta_\varepsilon$ é a função molificadora vista na Seção \ref{sec:aproximacoes}. Também podemos supor que para todo $k \in \bN$ e $\varepsilon > 0$ as funções $u_k^\varepsilon$ tem suporte em $\Omega'$. Afirmamos que
    \begin{equation} \label{eq:convergenciaemlq}
        \Vert u^\varepsilon_k - u_k \Vert_{\cL^q(\Omega')} \to 0
    \end{equation}
    quando $\varepsilon \to 0$ uniformemente em $k$.
    ...
    \]
    e integrando ambos os lados sobre $\Omega'$
    \[
        \begin{aligned}
            \int_{\Omega'} |u_k^\varepsilon(x) - u_k(x)|\,dx &\leqslant \int_{\Omega'} \varepsilon \int_{B(\bz,1)} \eta(y) \int_0^1 |Du_k(x - \varepsilon ty) \,dtdydx\\
            &\leqslant \varepsilon \int_{B(\bz,1)} \eta(y) \int_0^1 \int_{\Omega'} |Du_k(x - \varepsilon ty)| \,dxdtdy \leqslant \varepsilon \int_{\Omega'} |Du_k(z)| \,dz.
        \end{aligned}
    \]
    Isto é
    \[
        \Vert u_k^\varepsilon - u_k \Vert_{\cL^1(\Omega')} \leqslant \varepsilon \Vert Du_k \Vert_{\cL^1(V)} \leqslant \varepsilon c \Vert Du_k \Vert_{\cL^p(\Omega')} \leqslant \varepsilon c \Vert u_k \Vert_{\cW^{1,p}(\Omega')}
    \]
    Akém disso, utilizando (\ref{eq:supfinito}) obtemos que
    \begin{equation} \label{eq:eml1converge}
        \Vert u_k^\varepsilon - u_k \Vert_{\cL^1(\Omega')} \to 0
    \end{equation}
    ...
    Ademais, por () e (), $\Vert u_k^\varepsilon - u_k \Vert_{\cL^{p^*}(V)}^{1- \theta}$ é finito.
    ...
    \]
    quando $\varepsilon \to 0$ uniformemente em $k$. Como era desejado.
    ...
    \[
        |u_k^\varepsilon(x)| \leqslant \int_{B(x,\varepsilon)} \eta_\varepsilon (x - y) |u_k(y)| \,dy \leqslant \Vert \eta_\varepsilon \Vert_{\cL^\infty(\bR^n)} \Vert u_k \Vert_{\cL^1(V)} < \infty.
    \]
    De forma análoga, mostramos que $|Du_k^\varepsilon(x)| < \infty$.
    Isso prova a afirmação pois (...)
    ...
    \[
        \begin{aligned}
            \limsup_{j,\ell \to \infty} \Vert u_{k_j} - u_{k_\ell} \Vert_{\cL^q(V)} \leqslant \limsup_{j,\ell \to \infty} \Vert u_{k_j} - u_{k_j}^\varepsilon \Vert_{\cL^q(V)} + \limsup_{j,\ell \to \infty} \Vert u_{k_j}^\varepsilon - u_{k_\ell}^\varepsilon \Vert_{\cL^q(V)} + \limsup_{j,\ell \to \infty} \Vert u_{k_\ell}^\varepsilon - u_{k_\ell} \Vert_{\cL^q(V)}
        \end{aligned}
    \]
    como era desejado.
    Por fim, escolhendo $\delta = 1,\frac{1}{2},\dots$ em (\ref{eq:limsup}), conseguimos extrair uma subsequência $(u_{k_j})_{j=1}^\infty \subseteq (u_k)_{k=1}^\infty$ que satisfaz
    \[
