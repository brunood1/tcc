...
\begin{ex}
    Sejam $\mu$ uma medida em $\eth$ e $A \in \eth$ um conjunto fixo.
...
\begin{lbox}
    Seja $\mu$ uma medida.
    Se $E, F \in \eth$ com $E \subseteq F$, então $\mu(E) \leqslant \mu(F)$. 
\end{lbox}
\begin{prf}
    
\end{prf}

\obs ($\mu(E \smallsetminus F)$)
...
\obs Quando $p = 2$, a notação $H^{k}(\Omega)$ é comumente utilizada para dar ênfase que o espaço $\cW^{k,2}(\Omega)$ é um espaço de Hilbert\footnote{espaço vetorial completo com produto interno}
...
\obs $\cW^{k,p}_{\loc}(\Omega)$

\obs (ainda vamos verificar que é uma norma, precisamos de um teorema antes)
...
\begin{prf}
    ...
    {\small
    \[
        \int_\Omega \eta u D^{\alpha}\phi \, dx = (-1)^{|\beta|} \int_\Omega \left[\sum_{\sigma \leqslant \beta} \binom{\beta}{\sigma} D^{\sigma} \eta D^{\beta - \sigma} u\right] D^{\gamma}\phi \, dx = (-1)^{|\beta| + |\gamma|} \int_\Omega \left[\sum_{\sigma \leqslant \beta} \binom{\beta}{\sigma} D^{\gamma}(D^{\sigma} \eta D^{\beta - \sigma} u)\right] \phi \, dx
    \]}
    ...
    % {\small
    % \[
    %     \sum_{\gamma \leqslant \rho \leqslant \beta} \binom{\beta}{\rho - \gamma} D^{\rho}\eta D^{\alpha - \rho} u + \sum_{\beta < \rho \leqslant \alpha} \binom{\beta}{\rho - \gamma} D^{\rho}\eta D^{\alpha - \rho} u + \sum_{0 \leqslant \sigma < \beta} \binom{\beta}{\sigma} D^{\sigma} \eta D^{\alpha - \sigma} u + \sum_{\gamma \leqslant \sigma \leqslant \beta} \binom{\beta}{\sigma} D^{\sigma} \eta D^{\alpha - \sigma} u.
    % \]}
...
\begin{tbox}
    $(\cW^{k,p}(\Omega), \Vert \cdot \Vert_{\cW^{k,p}(\Omega)})$ com $1 \leqslant p < \infty$ é um espaço vetorial normado.
\end{tbox}
% \begin{prf}
    % Note que
    % \begin{enumerate}[leftmargin=*]
    %     \item $\Vert u \Vert_{\cW^{k,p}(\Omega)} = 0 \iff \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha u \Vert_p^p \right)^{\frac{1}{p}} = 0 \iff D^\alpha u = 0$ para todo $\alpha$ multi-índice.
    %     Em particular, se $\alpha = (0,\dots,0)$ $u = D^\alpha u = 0$.

    %     \item $\Vert \lambda u \Vert_{\cW^{k,p}(\Omega)} = \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha (\lambda u) \Vert_p^p \right)^{\frac{1}{p}} = \left( \sum_{|\alpha| \leqslant k} \Vert \lambda D^\alpha u \Vert_p^p \right)^{\frac{1}{p}} = \left( \sum_{|\alpha| \leqslant k} |\lambda|^p\Vert D^\alpha u \Vert_p^p \right)^{\frac{1}{p}} = |\lambda|\left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha u \Vert_p^p \right)^{\frac{1}{p}} = |\lambda| \Vert u \Vert_{\cW^{k,p}(\Omega)}$
        
    %     \item $\Vert u + v \Vert_{\cW^{k,p}(\Omega)} = \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha(u + v) \Vert_p^p \right)^{\frac{1}{p}} = \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha u + D^\alpha v \Vert_p^p \right)^{\frac{1}{p}}$
        
    %     $\leqslant \left( \sum_{|\alpha| \leqslant k} (\Vert D^\alpha u \Vert_p + \Vert D^\alpha v \Vert_p)^p \right)^{\frac{1}{p}} \leqslant \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha u \Vert_p^p \right)^{\frac{1}{p}} + \left( \sum_{|\alpha| \leqslant k} \Vert D^\alpha v \Vert_p^p \right)^{\frac{1}{p}}$
        
    %     $= \Vert u \Vert_{\cW^{k,p}(\Omega)} + \Vert v \Vert_{\cW^{k,p}(\Omega)}$
    % \end{enumerate}
    % Portanto $\Vert \cdot \Vert_{\cW^{k,p}(\Omega)}$ é uma norma.
% \end{prf}

\begin{tbox}
    $(\cW^{k,p}(\Omega), \Vert \cdot \Vert_{\cW^{k,p}(\Omega)})$ com $1 \leqslant p < \infty$ é completo
\end{tbox}
% \begin{prf}
%     Seja $(u_n)$ uma sequência de Cauchy em $\cW^{k,p}(\Omega)$.
%     Então, dado $\varepsilon > 0$ existe $n_0 \in \bN$ tal que
%     \[
...
\[
    ...
    \right.
\]
conhecida como molificador de Friedrich.
...
\]
essa função $\eta_\varepsilon$ é a função que será utilizada para realizar as convoluções que aproximam as funções em $\cW^{k,p}(\Omega)$
...
\begin{tbox}[Aproximação local por funções suaves]
    ...
    Então
    \begin{enumerate}[leftmargin=*, label=\textbf{(\alph*)}]
        \item $u^\varepsilon \in \cC^{\infty}(\Omega)$ para cada $\varepsilon > 0$
...
\begin{ex}
    A função $u(x) = |x|$ definida no intervalo aberto $I = (-1,1)$ é um exemplo clássico de função que não é diferenciável. É fácil verificar que $u \in \cW^{1,p}(I)$ (caso análogo ao que foi feito no Exemplo \ref{ex:derivada-fraca-R}). 
    Vamos utiizar a convolução para encontrar uma aproximação suave de $u$. 
    Seja $\eta : \bR \to \bR$ dada por
    \[
        \eta(x) = \left\{ 
            \begin{array}{lr}
                c \exp\left( -\frac{1}{1 - x^2} \right) & \text{se }\; |x| \leqslant 1\\
                0 & \text{se }\; |x| > 1
            \end{array}
        \right.
    \]
    ...
    isto é
    \[
        c = \left( \int_\bR \exp \left( -\frac{1}{1-x^2} \right) \, dx\right)^{-1}.
    \]
...
\begin{ex}
    ...
    \]
    não é diferenciável, mas possui derivada fraca em $\cW^{1,p}(\Omega)$ para $0 < p < 2$. Com efeito, note que
    \[
        \dots
    \]
    ...
    \[
        \eta(x) = \dots
    \]
    e $\eta_\varepsilon$ da mesma forma que foi feita no exemplo anterior. Novamente utilizaremos a convolução para encontrar uma aproximação suave para $u$.
    \[
...
\begin{tbox}
    ...
    Então existem funções $u_n \in \cC^{\infty} \cap \cW^{k,p}(\Omega)$ tais que
    \[
