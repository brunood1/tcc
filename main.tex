...
\begin{dbox}
    Sejam $I$ um intervalo aberto, possívelmente ilimitado e $u,g \in \cL^1_{\mathrm{loc}}$.
    Dizemos que $g$ é a derivada fraca de $u$, se
    \[
        \int_I u \phi' \, dx = -\int_I g \phi\,dx   
    \]
    para todo $\phi \in \cC^\infty_c(I)$.
\end{dbox}

Denotamos $u' = g$ e diremos $g$ é a derivada de $u$ no sentido fraco.
...
\begin{dbox}
    ...
    Ou
    \[
        \mathcal{W}^{k,p}(\Omega) = \left\{ u \in \mathcal{W}^{k-1,p}(\Omega) \,; D^\alpha u \in \mathcal{W}^{k-1,p}(\Omega) , \forall \alpha \text{ com } |\alpha| = 1\right\}
    \]
