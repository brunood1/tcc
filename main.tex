...
\begin{tbox}[Propiedades da transformada de Fourier] thm:propriedades-transformada
...
\[
    F = \int_{\bR^n} e^{i \omega \cdot x} e^{\nu t \Vert \omega \Vert^2} \,d\omega = 
\]
Portanto
\[
...
Em 1934 \cite{leray-fluid}, Leray construiu soluções fracas globais de energia finita
...
\begin{equation} \label{eq:navierstokes}
    ...
    \end{aligned}
\end{equation}
onde $\nu > 0$ é constante. Estas soluções reproduzem o estado inicial $\bu_0$ em $\cL^2$, ou seja, $\Vert \bu(\cdot,t) - \bu_0 \Vert_{\cL^2(\bR^3)} \to 0$ quando $t \to 0$ e satisfazem a desigualdade de energia
\begin{equation} \label{eq:desigualdade-de-energia}
    \Vert \bu(\cdot,t) \Vert_{L^2(\bR^3)}^2 + 2\nu \int_0^t \Vert D\bu(\cdot,s) \Vert_{\cL^2(\bR^3)}^2 \,ds \leqslant \Vert \bu_0 \Vert_{\cL^2(\bR^3)}^2.
\end{equation}
A unicidade dessas soluções dada um estado inicial $\bu_0 \in \cL^2_\sigma(\bR^3)$ ainda é um problema em aberto, porem Leray mostrou que existe um instaante de tempo $0 < T_{**} < \infty$ (que depende de $\nu$ e $\bu_0$) tal que $\bu \in \cC^{\infty}(\bR^3 \times [T_{**}, \infty))$ e $\bu(\cdot,t) \in \cL^{\infty}_{\loc}([T_{**},\infty), \cH^k(\bR^3))$ para cada $k \geqslant 0$.
No final do artigo \cite{leray-fluid}, Leray deixou um problema em aberto, perguntando se
\[
    \Vert D\bu(\cdot,t) \Vert_{\cL^2(\bR^3)} \to 0
\]
quando $t \to \infty$. A afirmação acima foi provada 50 anos depois por Kato [??] e depois por outros autores. Uma demonstração para o problema de Leray será apresentada no Teorema \ref{thm:problema-leray} no final dessa seção.
...
\begin{prf}
    ...
    onde
    \[
        J_{\alpha,\delta}(t) = \left\Vert D^\alpha\left( e^{\nu\Delta(t-\tilde t_0)} \big[\bu(\cdot,\tilde t_0) - \bu_\delta(\cdot,\tilde t_0)\big] \right) \right\Vert _{\cL^2(K)} \!\!+ \left\Vert D^\alpha\left( e^{\nu\Delta(t-t_0)} \big[\bu(\cdot, t_0) - \bu_\delta(\cdot, t_0)\big] \right) \right\Vert _{\cL^2(K)}.
    \]
