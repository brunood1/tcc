...
\usepackage{csquotes}
\usepackage[style=abnt, backend=biber, maxbibnames=5, minbibnames=3, sorting=nty, url=false, isbn=false, doi=false, defernumbers=true]{biblatex}
...
\end{tbox}
\begin{prf}
    \textcolor{red}{preciso de uma referencia}
\end{prf}
...
\]
mas como $\phi$ se anula em $\partial \Omega$, obtemos
\[
    \int_\Omega -\phi \Delta u \,dx = \sum_{i=1}^n \int_\Omega \frac{\partial \phi}{\partial x_i} \frac{\partial u}{\partial x_i} \,dx = \int_{\Omega} \nabla u \cdot \nabla \phi \,dx.
\]
Dito isso, dizemos que $u$ é uma solução fraca do problema de Dirichlet se existe uma função $\phi \in \cC^{\infty}_C(\Omega)$\footnote{Nomenclatura: $\phi$ é chamada função teste} tal que
...
Na Seção \ref{eq:sol-fraca-dirichlet} voltaremos a essa motivação para mostrar que para qualquer função $f \in \cL^2(\Omega)$, existe uma única solução fraca para (\ref{eq:problema-dirichlet-motivacao}). 
...
\end{lbox}
\begin{prf}
    \cite{brezis-functional.analysis}
\end{prf}
...
\begin{ex}
    ...
    De fato, integrando $\Vert Du \Vert$, temos
    \[
    ...
    \[
        \int \frac{1}{r^{\alpha+1}}  \int_{\Vert x \Vert = r} dS dr = \sigma(n)\int_0^1 r^{n-\alpha-2} \,dr = \sigma(n)\left(\frac{1^{n-\alpha-1}}{n-\alpha -1} - \frac{0^{n-\alpha-1}}{n-\alpha-1}\right).
    \]
...
Nessa seção veremos uma forma de extender funções em $\cW^{1,p}(\Omega)$ em funções $\cW^{1,p}(\bR^n)$ por meio de um operador linear.
...
\begin{prf}
    ...
    \]
    para todo $u \in \cW^{1,p}(\Omega)$
    \textcolor{red}{esss demonstração tem uma parte final que eu não entendi direito}
\end{prf}
...
\begin{prf}
    ...
    \[
        ...
        \end{aligned}
    \]
    onde $c$ é uma constante que depende de $p$ e $n$. Daí utilizando a desigualdade entre a derivada parcial e a norma do gradiente
    \[
...
\begin{prf}
    ...
    \[
        |u(x + sw) - u(x)| = \left| \int_0^s u'(x + tw) \,dt \right| = \left|\int_0^s Du(x + tw) \cdot w \,dt \right| \leqslant \int_0^s \Vert Du(x + tw) \Vert \,dt
    \]
    ...
    {\small
    \[
        \int_{\partial B(\bz,1)} |u(x + sw) - u(x)|\,dS \leqslant \int_0^s \int_{\partial B(\bz,1)} \Vert Du(x + tw) \Vert \,dSdt = \int_0^s \int_{\partial B(\bz,1)} \Vert Du(x + tw) \Vert \frac{t^{n-1}}{t^{n-1}} \,dSdt.
    \]}\!
    Seja $y = x + tw$, de forma que $t = \Vert x - y \Vert$. Assim, por meio de coordenadas polares obtemos
    \[
        \int_{\partial B(\bz,1)} |u(x + sw) - u(x)|\,dS \leqslant \int_0^s \int_{B(x,t)} \frac{\Vert Du(y) \Vert}{|x-y|^{n-1}} \,dSdt =  \int_{B(x,s)} \frac{\Vert Du(y) \Vert}{|x-y|^{n-1}} \,dy
    \]
    e como $s < r$
    \[
        \int_{\partial B(\bz,1)} |u(x + sw) - u(x)|\,dS \leqslant   \int_{B(x,r)} \frac{\Vert Du(y) \Vert}{|x-y|^{n-1}} \,dy.
    \]
    ...
    \[
        \sint_B |u(x) - u(z)| \,dz \leqslant \sint_{B(x,r)} |u(x) - u(z)| \,dz \leqslant c \int_{B(x,r)} \frac{\Vert Du(z) \Vert}{|z-x|^{n-1}} \,dz
    \]
...
\end{tbox}
\begin{prf}
    \cite{evans-pde} p.p. 183
\end{prf}

% Utilizando o Teorema acima, podemos definir a transformada de Fourier para funções em $\cL^2(\bR^n)$.
% Para isso, considere uma sequência $(u_k)_{k=1}^\infty  \subseteq \cL^1(\bR^n) \cap \cL^2(\bR^n)$ tal que $u_k \to u$ em $\cL^2(\bR^n)$. Dito Isso
% \[
%     \Vert \hat u_k - \hat u_j \Vert_{\cL^2} = \Vert u_k - u_j \Vert_{\cL^2(\bR^n)}
% \]
% ou seja, $(\hat u_k)$ é uma sequência de Cauchy em $\cL^2$.
% Logo é convergente, e esse limite é o que chamamos de $\hat u$.
% De forma análoga, definimos $\check u$.
...
\end{pbox}
\begin{prf}
    \cite{lorenz-navier.stokes} p.p. 32
\end{prf}
...
\]
Quando escrevemos $\bu(\cdot,t)$ estamos desconsiderando o tempo como uma variável (pense como um $t$ fixo) (escrever melhor)

\section{Propriedades da soluções de Leray}

\subsection{Introdução e contexto histórico}

Em 1934 no artigo \textit{``Sur le mouvement d'un liquide visqueux emplassement l'espace''} \cite{leray-fluid} Leray construiu soluções fracas de energia finita\footnote{i.e.,}
...
\end{equation}
A unicidade desas soluções ainda é um problema em aberto, porem no mesmo artigo Leray motrou que existe um instante de tempo $T_{**}$ tal que a solução $\bu$ se torna suave em $\bR^3 \times [T_{**}, \infty)$ e $\bu(\cdot,t) \in \cL^{\infty}_{\loc}\big( [T_{**}, \infty), H^k(\bR^3) \big)$\footnote{i.e.,} para cada $k \geqslant 0$.
...
\begin{prf}
    ...
    Dito isso, seja $w \in \ker f ^\perp$ não nulo, para cada $u \in H$, $f(u) w - f(w)u \in \ker f$ já que $f(f(u)w - f(w)u) = f(u) f(w) - f(w)f(u) = 0$.
    Dessa forma
    \[
    ...
    \]
    Denotando $v := \frac{f(w)}{\Vert w \Vert^2} w \in H$ (note que $v$ não depende de $u$), temos que existe $v \in H$ tal que $f(u) = \left\langle u,v \right\rangle$ para todo $u \in H$.

    Para mostrar que $v$ é único, suponha que exista $\tilde v \in H$ tal que $f(u) = \left\langle u, \tilde v \right\rangle$ para todo $u \in H$.
    Dito isso $\left\langle u, v \right\rangle = f(u) = \left\langle u,\tilde v \right\rangle$ para todo $u \in H$, reescrevendo, temos $\left\langle u , v - \tilde v \right\rangle = 0$ para todo $u \in H$, em particular se $u = v - \tilde v \in H$, temos $\Vert v - \tilde v \Vert = 0$, ou seja $v = \tilde v$. Portanto $v$ é único.
    ...
    Por outro lado, por Cauchy-Schawarz
    \[
...
\begin{tbox}[Teorema de Lax-Milgram]
    ...
    \end{enumerate}
    e $f : H \to \bR$ um funcional linear limitado. Então existe um único $v \in H$ tal que $B(u,v) = f(u)$ para todo $u \in H$.
\end{tbox}
\begin{prf}
    Essa demonstração sera dividida em alguns passos
    ...
    \]
    para todo $u \in H$, onde utilizamos o fato de $B$ ser limitada.
    Sendo assim
    \[
        |g_v(u)| \leqslant \alpha \Vert u \Vert
    \]
    para todo $u \in H$.
    Logo $g_v$ é limitada.
    Dito isso, pelo Teorema da Representação de Riesz, existe um único $w_v \in H$ tal que
    \[
    ...
    \underline{Passo 2:} $A$ é um operador linear e limitado
    ...
    Logo $A$ é linear.
    Além disso, dado $v \in H$
    \[
        \Vert Av \Vert^2 = \left\langle Av, Av \right\rangle = B(Av, v) \leqslant \alpha \Vert Av \Vert \Vert v \Vert
    \]
    como $Av \neq 0$ obtemos
    \[
    ...
    \underline{Passo 3:} $A$ é injetivo e $\mathrm{Im}A$ é fechado

    Sabemos que existe $\beta > 0$ tal que $\beta \Vert u \Vert^2 \leqslant |B(u,u)|$. Dessa forma
    \[
    ...
    Portanto $u$ é injetiva.
    Além disso, seja $(Au_k) \subseteq \mathrm{Im} A$ tal que $A u_k \to v \in H$.
    ...
    Como $H$ é completo, existe $u_0 \in H$ tal que $u_n \to u_0$. Como $A$ é limitado (o que implica em $A$ ser contínuo) temos $Au_k \to Au_0$.
    Pela unicidade do limite $v = Av_0 \in \mathrm{Im}A$.
    Portanto $\mathrm{Im}A$ é fechado.

    \underline{Passo 4:} $A$ é sobrejetivo

    Sabemos que $H = \mathrm{Im}A \oplus \mathrm{Im}A^\perp$. Mostremos que $\mathrm{Im}A^\perp = \{0\}$.
    Com efeito, se $w \in \mathrm{Im}A^\perp$, temos que
    \[
        \beta \Vert w \Vert^2 \leqslant | B(w,w) | = | \left\langle w, Aw \right\rangle | = 0
    \]
    Logo, $w = 0$ o que implica em $H = \mathrm{Im}A$.
    Portanto $A$ é sobrejetiva.

    \underline{Passo 5:} $f(u) = B(u,v)$

    Como $f$ é um funcional linear limitado em $H$, pelo Teorema da Representação de Riesz existe $z \in H$ tal que $f(u) = \left\langle u, z \right\rangle$ para todo $u \in H$.
    Como $A$ é uma bijeção, existe um único $v \in H$ tal que $z = Av$, dessa forma
    \[
        f(u) = \left\langle u,z \right\rangle = \left\langle u, Av \right\rangle = B(u,v)
    \]
    para todo $u \in H$.

    \underline{Passo 6:} Unicidade

    Por fim, para mostrar que $v$ é único, suponha que exista $\tilde v \in H$ tal que $f(u) = B(u, \tilde v)$ para todo $u \in H$.
    Dessa forma
    \[
        B(u,v) = f(u) = B(u,\tilde v).
    \]
    Reescrevendo, temos $B(u, v - \tilde v) = 0$ para todo $u \in H$.
    Dito isso por (\ref{eq:uau})
    \[
        \beta \Vert v - \tilde v \Vert \leqslant | B(v - \tilde v, v - \tilde v) | = 0
    \]
...
\]
onde $\phi \in \cC^{\infty}_c(\Omega)$.
Porem pela densidade das funções teste em $H^{1}_0(\Omega)$, podemos dizer que $u$ é uma solução fraca do problema de Dirichlet se satisfaz
\[
    \int_\Omega Du \cdot Dv \,dx + \int_\Omega uv \,dx = \int_\Omega fv \,dx
\]
onde $v \in H_0^1(\Omega)$.

Defina $B : H^1_0(\Omega) \times H^1_0(\Omega) \to \bR$ por
\[
    B[u,v]=\int_\Omega Du \cdot Dv \,dx + \int_{\Omega} uv \,dx
\]
e $\varphi : H^1_0(\Omega) \to \bR$ por
\[
    \varphi(v) = \int_\Omega fv \,dx. 
\]
Note que $B$ é bilinear limitado e coercivo. Com efeito, a bilinearidade de $B$ segue do fato do gradiente fraco $D$ e a integral serem operadores lineares. Além disso
\[
    B(u,u) = \int_\Omega Du \cdot Du \,dx + \int_\Omega u^2 \,dx
    = \int_\Omega \Vert Du \Vert^2 \,dx + \int_\Omega |u|^2 \,dx 
    = \Vert u \Vert_{H^1_0(\Omega)}
\]
...
\[
    ...
    \end{aligned}
\]
Porem, sabemos que $\Vert u \Vert_{\cL^2(\Omega)}, \Vert Du \Vert_{\cL^2(\Omega)} \leqslant \Vert u \Vert_{H^1_0(\Omega)}$.
Dito isso, obtemos
\[
    |B(u,v)| \leqslant c \Vert u \Vert_{H^1_0(\Omega)} \Vert v \Vert_{H^1_0(\Omega)}
\]
para todo $u, v \in H^1_0(\Omega)$. Logo, $B$ é limitado.
Por fim, temos que $\varphi$ é limitado.
De fato
\[
...
Portanto existe uma única solução fraca para o problema de Dirichlet.
